...
class TransactionYear(View):
    def get(self, request):
        ...
        products_2019 = Transaction_product.objects.filter(transaction__date__year=2019)
        ...
        for trans_data in products_2019:
...
class TransactionSeason(View):
    ...
    def post(self, request):
        ...
        if req_season != "冬季":
            ...
            products_2019 = Transaction_product.objects.filter(transaction__date__year=2019, transaction__date__month__gte=month_start, transaction__date__month__lte=month_end)
            for trans_data in products_2017:
                product_dict[trans_data.product.name][0] += trans_data.quantity
            for trans_data in products_2018:
                product_dict[trans_data.product.name][1] += trans_data.quantity
            ...
            Q(transaction__date__month=1) | Q(transaction__date__month=2)))
            products_2019 = Transaction_product.objects.filter(Q(transaction__date__year=2019) & (Q(transaction__date__month=12) |
            Q(transaction__date__month=1) | Q(transaction__date__month=2)))
            for trans_data in products_2017:
                product_dict[trans_data.product.name][0] += trans_data.quantity
            for trans_data in products_2018:
...
class TransactionMonth(View):
    ...
    def post(self, request):
        ...
        products_2019 = Transaction_product.objects.filter(transaction__date__year=2019, transaction__date__month=req_month)
        for trans_data in products_2017:
            product_dict[trans_data.product.name][0] += trans_data.quantity
        for trans_data in products_2018:
            product_dict[trans_data.product.name][1] += trans_data.quantity
...
class TransactionChart(View):
    ...
    def post(self, request):
        ...
        chart = {
            ...
            'title': {
                'text': str(year) + '年 ' + str(month) + '月小雞麵銷售分布',
                'style': {
